...
namespace GOS_Server_A3.Classes
{
    ...
    {        
        ...
        static public string VersionSynchro()
        {
            ...
            try
            {
                XmlTextReader fichierInfoServer = new XmlTextReader(Var.fenetrePrincipale.textBox18.Text + @"\@GOS\version.xml");
        ...
        static private string DefRepertoireDeSauvegarde()
        {
            ...
            return Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData).ToString() + @"\GOSServerA3\"; 
        }

        static public int IndexProfilParDefaut()
        {
                int index = 0;
                string profilFavoris = Core.GetKeyValue(@"Software\Clan GOS\GOS Server A3\", "profil_favoris");
                if (profilFavoris != "00")
                {
                    int compteur = 0;
